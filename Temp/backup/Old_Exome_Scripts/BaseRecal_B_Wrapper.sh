...
#  Description : Job Submission Script - Base Recalibration Substage B                      |
#-------------------------------------------------------------------------------------------#
...
cp $5/'GATK_Pipeline'/$4'_recal_data.table' $TMPDIR
...
echo 'Recal Data: '$4'_recal_data.table' >> $5/$4'.log'
echo 'Running BaseRecalibrator: Second Pass' >> $5/$4'.log'
...
java -Xmx35g -jar \
    $GATK_ROOT/GenomeAnalysisTK.jar \
        ...
        -BQSR $TMPDIR/$4'_recal_data.table' \
        -o $TMPDIR/$4'_post_recal_data.table'

mv $TMPDIR/$4'_post_recal_data.table' $5/'GATK_Pipeline'
##'-----------------------------------------------------------------------------------------#

##'Add to Log
##'-----------------------------------------------------------------------------------------#
echo 'GATK Finished: '$(date) >> $5/$4'.log'
echo 'Post Recal Table Written to WORKING_DATA' >> $5/$4'.log'
##'-----------------------------------------------------------------------------------------#
