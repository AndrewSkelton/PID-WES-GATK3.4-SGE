...
#  Description : Job Submission Script - SelectVariants for SNPs                          |
#-------------------------------------------------------------------------------------------#
...
##'-----------------------------------------------------------------------------------------#
cp $3/'GATK_Pipeline'/$2'_Recal_SNPs.vcf'* $TMPDIR
...
##'-----------------------------------------------------------------------------------------#
echo '' >> $3/$2'.log'
echo 'GATK Started - SelectVariants (SNPs): '$(date) >> $3/$2'.log'
echo 'Parameters:' >> $3/$2'.log'
echo 'Reference Fa: ucsc.hg19.fasta' >> $3/$2'.log'
echo 'VCF In: '$2'_Recal_SNPs.vcf' >> $3/$2'.log'
echo 'Running SelectVariants (SNPs)' >> $3/$2'.log'

echo 'Files on Scratch:' >> $3/$2'.log'
echo `ls $TMPDIR` >> $3/$2'.log'
##'-----------------------------------------------------------------------------------------#

##'Run GATK: Haplotype Caller - SelectVariants SNP
##'-----------------------------------------------------------------------------------------#
java -Xmx4g -jar \
...
mv $TMPDIR/$2'_SNPs_selected.vcf'* $3/'GATK_Pipeline'
mv $3/'GATK_Pipeline'/$2'_Recal_SNPs.vcf'* $3/'GATK_Pipeline/VCF_Backups'
##'-----------------------------------------------------------------------------------------#
